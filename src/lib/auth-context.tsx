...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Auto-ensure the current calendar month exists whenever any user signs in.
  // Super-admins will succeed; non-privileged users will silently no-op when
  // Firestore rules block the write (handled by safeEnsureCurrentMonth).
  useEffect(() => {
    if (!profile) return;
    let cancelled = false;
    (async () => {
      try {
        const { safeEnsureCurrentMonth } = await import("@/lib/months");
        ...
        if (cancelled) return;
        await safeEnsureCurrentMonth(day, profile.language === "am" ? "am-ET" : profile.language === "om" ? "om-ET" : "en-US");
      } catch {
        // best effort
      }
    })();
